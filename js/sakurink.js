...
/* sidebar 乾坤大挪移（原本移动端底部那堆东西在有滑动侧栏时丢进去） */
(function () {
  let isMoved = false;
  ...
  function moveAsideContent() {
    ...
    const sidebarMenus = document.querySelector('#sidebar-menus .menus_items');
    const placeholder = document.getElementById('aside-content-placeholder');

    if (!aside || !sidebarMenus) return;
    ...
    if (isMobile && !isMoved) {
      ...
      ph.id = 'aside-content-placeholder';
      aside.parentNode.insertBefore(ph, aside);

      // 创建包裹容器
      const wrapper = document.createElement('div');
      wrapper.id = 'mobile-aside-wrapper';
      wrapper.className = 'mobile-aside-wrapper';
      wrapper.appendChild(aside); // 把原 aside 移入 wrapper

      // 插入到 sidebar-menus 后面（或按需调整位置）
      sidebarMenus.parentNode.insertBefore(wrapper, sidebarMenus.nextSibling);

      isMoved = true;
    } else if (!isMobile && isMoved) {
      // 移回原位置
      if (placeholder && placeholder.parentNode) {
        const wrapper = document.getElementById('mobile-aside-wrapper');
        if (wrapper && wrapper.firstElementChild === aside) {
