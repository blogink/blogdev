...
var tasseles = {
  ...
  changeMusicBg: function (isChangeBg = true) {
    ...
    function anMusicBgAddon() {
      const anMusicBg = document.getElementById("an_music_bg");
      const originalWebBg = document.getElementById("web_bg");

      if (!anMusicBg) {
        const anMusicBg = document.createElement("div");
        anMusicBg.id = "an_music_bg";
        anMusicBg.style.transition = "background-image 1s ease"; // 添加过渡效果

        // 使用 insertBefore 方法
        if (originalWebBg.nextSibling) {
          originalWebBg.parentNode.insertBefore(anMusicBg, originalWebBg.nextSibling);
        } else {
          // 如果没有下一个兄弟节点，直接添加到父元素的末尾
          originalWebBg.parentNode.appendChild(anMusicBg);
        }
      }
    }
    ...
    if (isChangeBg) {
      ...
      const observer = new MutationObserver(() => {
        ...
        if (musiccover) {
          observer.disconnect();
          anMusicBg.style.backgroundImage = musiccover.style.backgroundImage;
          tasseles.addEventListenerChangeMusicBg();
      ...
      // 开始观察目标节点的变化
      observer.observe(document.querySelector("#anMusic-page"), {
        childList: true,
        subtree: true
      });
    }
